    ...
    : BaseOAuthProvider(oauthSettings, logger), IOAuthProvider
{
        ...
        Uri? returnUrl)
    {
        ...
        Logger.LogInformation("Generated Telegram challenge URL: {Url}", (url ?? string.Empty)[..Math.Min(url?.Length ?? 0, 100)] + "...");
    ...
    public override async Task<OAuthUserInfoDto> GetUserInfoAsync(string accessToken, string? idToken, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(accessToken) && accessToken.StartsWith('{') && accessToken.EndsWith('}'))
            {
                ...
                userData = JsonSerializer.Deserialize<Dictionary<string, JsonElement>>(accessToken ?? string.Empty)
    ...
    kvp => kvp.Value.ValueKind == JsonValueKind.String
        ...
        catch (Exception ex)
        {
            ...
            {
                ["first_name"] = accessToken ?? string.Empty,
                ["last_name"] = "User",
            ...
            return new OAuthUserInfoDto
            {
                Id = accessToken ?? string.Empty, // Use the raw input as ID fallback for debugging
                Email = null,
                Name = "Telegram User",
                Username = "telegram_user",
                AvatarUrl = null,

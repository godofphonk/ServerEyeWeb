...
# Memory Snapshot Under Load Script for ServerEye Backend
# Creates memory snapshot with 60 seconds profiling under load
...
DURATION=${1:-60}  # Default 60 seconds, can be overridden

echo "🔍 Starting memory profiling under load for ${CONTAINER_NAME}..."
...
# Create snapshot with profiling
echo "📸 Creating memory snapshot with ${DURATION}s profiling..."
echo "💡 Start your load test in Postman NOW!"
echo ""

docker exec ${CONTAINER_NAME} sh -c "./dotMemoryclt/tools/dotmemory attach 1 --timeout=00:0${DURATION}:00"
...
echo "✅ Memory snapshot under load completed!"

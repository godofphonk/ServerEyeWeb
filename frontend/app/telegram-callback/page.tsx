        ...
        try {
          ...
          const requestBody = {
            UserData: {
              ...
              AuthDate: parseInt(
                telegramData.auth_date || telegramData.AuthDate || Date.now().toString(),
              ),
          ...
          fetch(`${process.env.NEXT_PUBLIC_API_URL! + '/api'}/api/auth/oauth/telegram/callback`, {
            method: 'POST',
            headers: {
            ...
            .then(data => {
              ...
              if (data.success && (data.token || data.refreshToken)) {
                ...
                if (linkingInfo) {
                  if (
                    data.message &&
                    data.message.toLowerCase().includes('already linked to another user')
                  ) {
                    window.location.href = '/profile?error=already_linked';
                  } else if (
                    data.message &&
                    data.message.toLowerCase().includes('already linked')
                  ) {
                    window.location.href = '/profile?error=already_linked';
                  } else {

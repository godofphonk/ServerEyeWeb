...
test.describe('Server Management Flow', () => {
  ...
  test('should display server list page after login', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="add-server"], button:has-text("Add"), a:has-text("Add")')).toBeVisible();
  });
  ...
  test('should navigate to add server page', async ({ page }) => {
    ...
    const addButton = page.locator('[data-testid="add-server"], button:has-text("Add Server"), a[href*="add"]');
  ...
  test('should show add server form with required fields', async ({ page }) => {
    ...
    await expect(page.locator('input[name="hostname"], input[name="ip"], input[placeholder*="IP" i]')).toBeVisible();
  ...
  test('should validate required fields on add server form', async ({ page }) => {
    ...
    await expect(page.locator('text=/required|Required|error|Error/i').or(page.locator('.error, [role="alert"]'))).toBeVisible();
  });
  ...
  test('should show server connection options', async ({ page }) => {
    ...
    const connectionOptions = page.locator('[data-testid="connection-method"], button:has-text("SSH"), button:has-text("Agent"), button:has-text("Manual")');
  ...
  test('should display server details page', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="metrics"], text=/metrics|Metrics|stats|Statistics/i').or(page.locator('.metrics'))).toBeVisible();
  });
  ...
  test('should show server actions (edit, delete)', async ({ page }) => {
    ...
    const serverItems = page.locator('[data-testid="server-item"], .server-card, tr:has-text("server")');
    const count = await serverItems.count();
    
    if (count > 0) {
      // Check for action buttons on first server
      const firstServer = serverItems.first();
      await expect(firstServer.locator('button:has-text("Edit"), a:has-text("Edit")')).toBeVisible();
      await expect(firstServer.locator('button:has-text("Delete"), a:has-text("Delete")')).toBeVisible();
    }
  });
  ...
  test('should open delete server confirmation', async ({ page }) => {
    ...
    // Find delete button
    const deleteButton = page.locator('button:has-text("Delete"), [data-testid="delete-server"]').first();
    
    if (await deleteButton.isVisible().catch(() => false)) {
      await deleteButton.click();
      
      // Check confirmation dialog/modal
      await expect(page.locator('text=/confirm|Confirm|delete|Delete/i').locator('visible=true')).toBeVisible();
    }
  });
...
test.describe('Server Discovery', () => {
  ...
  test('should show server discovery option', async ({ page }) => {
    ...
    const discoveryButton = page.locator('button:has-text("Discover"), button:has-text("Scan"), [data-testid="discovery"]');
  ...
  test('should show Telegram server discovery', async ({ page }) => {
    ...
    const telegramButton = page.locator('button:has-text("Telegram"), [data-testid="telegram-discovery"]');

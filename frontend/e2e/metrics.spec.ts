...
test.describe('Server Metrics View', () => {
  ...
  test('should display server metrics page', async ({ page }) => {
    ...
    const metricsSection = page.locator(
      '[data-testid="metrics"], .metrics-container, section:has-text("Metrics")',
    );
  ...
  test('should show CPU usage chart', async ({ page }) => {
    ...
    await expect(
      page.locator('.recharts-wrapper, canvas, svg:has(rect, circle, path)'),
    ).toBeVisible();
  });
  ...
  test('should have time range selector', async ({ page }) => {
    ...
    const timeSelector = page.locator(
      '[data-testid="time-range"], select:has-option("1h"), button:has-text("1h"), button:has-text("24h")',
    );
  ...
  test('should change time range', async ({ page }) => {
    ...
    const timeButtons = page.locator(
      'button:has-text("1h"), button:has-text("6h"), button:has-text("24h"), button:has-text("7d")',
    );
  ...
  test('should refresh metrics manually', async ({ page }) => {
    ...
    const refreshButton = page.locator(
      '[data-testid="refresh"], button:has-text("Refresh"), button[title*="refresh" i]',
    );
  ...
  test('should show live/real-time indicator', async ({ page }) => {
    ...
    const liveIndicator = page.locator(
      'text=/live|Live|real-time|Real-time/i, [data-testid="live"], .pulse, .online-indicator',
    );
...
test.describe('Dashboard Metrics Overview', () => {
  ...
  test('should display dashboard with metrics summary', async ({ page }) => {
    ...
    await expect(
      page
        .locator('[data-testid="server-status"], .status-card, .server-card')
        .or(page.locator('text=/servers online|active|status/i')),
    ).toBeVisible();
  });
  ...
  test('should show total servers count', async ({ page }) => {
    ...
    await expect(
      page
        .locator('text=/[0-9]+ servers?/i')
        .or(page.locator('[data-testid="server-count"]'))
        .or(page.locator('.metric:has-text("Server")')),
    ).toBeVisible();
  });
  ...
  test('should show alerts/notifications count', async ({ page }) => {
    ...
    await expect(
      page
        .locator('text=/alert|Alert|notification|Notification/i')
        .or(page.locator('[data-testid="alerts"]')),
    ).toBeVisible();
  });
  ...
  test('should navigate to server details from dashboard', async ({ page }) => {
    ...
    const serverLink = page
      .locator('[data-testid="server-card"] a, .server-card a, a[href*="/servers/"]')
      .first();
...
test.describe('Admin Monitoring', () => {
  ...
  test('should show system health status', async ({ page }) => {
    ...
    const healthIndicator = page.locator(
      'text=/healthy|Healthy|status|Status/i, [data-testid="health"], .health-status',
    );

    if (
      await healthIndicator
        .first()
        .isVisible()
        .catch(() => false)
    ) {

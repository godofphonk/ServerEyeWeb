...
test.describe('Pricing Page', () => {
  ...
  test('should display Pro/Paid plan with price', async ({ page }) => {
    ...
    const priceElement = page.locator('text=/\\$[0-9]+|€[0-9]+|[0-9]+\$/i').or(
      page.locator('.price:has-text("$")')
    );
    
    if (await priceElement.first().isVisible().catch(() => false)) {
  ...
  test('should have upgrade/ subscribe buttons', async ({ page }) => {
    ...
    await expect(page.locator('button:has-text("Upgrade"), button:has-text("Subscribe"), button:has-text("Get Started"), a:has-text("Upgrade")').first()).toBeVisible();
  });
  ...
  test('should toggle monthly/yearly pricing', async ({ page }) => {
    ...
    const toggle = page.locator('[data-testid="billing-toggle"], button:has-text("Monthly"), button:has-text("Yearly"), .toggle');
    
    if (await toggle.first().isVisible().catch(() => false)) {
      // Get initial price
      const initialPrice = await page.locator('.price').first().textContent().catch(() => '');
      ...
      // Price should change
      const newPrice = await page.locator('.price').first().textContent().catch(() => '');
...
test.describe('Checkout Flow', () => {
  ...
  test('should navigate to checkout from pricing', async ({ page }) => {
    ...
    const upgradeButton = page.locator('[data-testid="pro-plan"] button, .pro-plan button, button:has-text("Upgrade to Pro")').first();
  ...
  test('should display checkout page with order summary', async ({ page }) => {
    ...
    await expect(page.locator('text=/order summary|summary|total|plan/i').or(
  ...
  test('should show Stripe payment form', async ({ page }) => {
    ...
    const stripeFrame = page.locator('iframe[src*="stripe"], [data-testid="card-element"], .StripeElement');
  ...
  test('should require card details for payment', async ({ page }) => {
    ...
    if (await submitButton.isVisible().catch(() => false)) {
      ...
      // Should show error
      await expect(page.locator('text=/required|invalid|error|Error/i').or(
  ...
  test('should handle Stripe test card payment', async ({ page }) => {
    ...
    if (await cardFrame.locator('input').first().isVisible().catch(() => false)) {
...
test.describe('Subscription Management', () => {
  ...
  test('should display billing/subscription page', async ({ page }) => {
    ...
    const billingLink = page.locator('a:has-text("Billing"), a:has-text("Subscription"), button:has-text("Billing"), [data-testid="billing"]').first();
  ...
  test('should show current subscription status', async ({ page }) => {
    ...
    await expect(page.locator('text=/plan|Plan|subscription|Subscription|status|Status/i').first()).toBeVisible();
  });
  ...
  test('should show payment methods', async ({ page }) => {
    ...
    const paymentMethods = page.locator('text=/payment method|card|Card/i, [data-testid="payment-methods"]');
  ...
  test('should show billing history/invoices', async ({ page }) => {
    ...
    const invoices = page.locator('text=/invoice|history|billing history|payment history/i, [data-testid="invoices"]');
    
    if (await invoices.first().isVisible().catch(() => false)) {
  ...
  test('should have cancel subscription option', async ({ page }) => {
    ...
    const cancelButton = page.locator('button:has-text("Cancel"), button:has-text("Unsubscribe"), [data-testid="cancel-subscription"]');
  ...
  test('should handle subscription cancellation flow', async ({ page }) => {
    ...
    const cancelButton = page.locator('button:has-text("Cancel Subscription"), button:has-text("Unsubscribe")').first();
    
    if (await cancelButton.isVisible().catch(() => false)) {
      await cancelButton.click();
      
      // Should show confirmation dialog
      await expect(page.locator('text=/confirm|Confirm|are you sure/i').or(
  ...
  test('should update payment method', async ({ page }) => {
    ...
    const updateButton = page.locator('button:has-text("Update"), button:has-text("Change Card"), [data-testid="update-payment"]').first();
    
    if (await updateButton.isVisible().catch(() => false)) {
      await updateButton.click();
      
      // Should show payment form
      await expect(page.locator('iframe[src*="stripe"], [data-testid="card-element"]').or(
...
test.describe('Webhook Handling', () => {
  ...
  test('should show upgrade success message after return from Stripe', async ({ page }) => {
    ...
    await expect(page.locator('text=/success|Success|thank you|Thank you|welcome|Welcome/i').or(

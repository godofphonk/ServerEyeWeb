    ...
    <Card className='p-6 border-blue-500/20 bg-gradient-to-br from-blue-500/5 to-purple-500/5'>
        ...
        <div className='flex items-center gap-2'>
          {isConnected ? (
            ...
            <>
              ...
              <span className='text-sm text-red-400 font-semibold'>{error || 'Disconnected'}</span>
            </>
          )}
        ...
        {lastMessage ? (
          ...
          >
            ...
            <div className='bg-white/5 rounded-lg p-4'>
              ...
              <p
                className={`text-2xl font-bold ${getMetricColor(lastMessage?.memory || 0, 'memory')}`}
              >
            ...
            <div className='bg-white/5 rounded-lg p-4'>
              ...
              <p
                className={`text-2xl font-bold ${getMetricColor(lastMessage?.temperature || 0, 'temperature')}`}
              >
            ...
            <div className='bg-white/5 rounded-lg p-4'>
              ...
              <p
                className={`text-2xl font-bold ${getMetricColor(lastMessage?.gpu_temperature || 0, 'temperature')}`}
              >
      ...
      {lastMessage && (
        <div className='mt-4 text-xs text-gray-500 text-center'>
          Last update:{' '}
          {new Date(lastMessage?.timestamp || new Date().toISOString()).toLocaleTimeString()}
        </div>
      )}

      <MetricsNotification message={apiMessage} onClose={() => setApiMessage(null)} />
    </Card>
  );
}

...
export function ServerError({ 
    ...
    switch (status) {
      case 500:
        return {
          ...
          description: 'Произошла внутренняя ошибка на сервере. Мы уже работаем над решением проблемы.',
      ...
      case 502:
        return {
          ...
          description: 'Сервер получил некорректный ответ от вышестоящего сервиса. Попробуйте повторить запрос.',
      ...
      case 503:
        return {
          ...
          description: 'Сервис временно недоступен из-за технических работ. Пожалуйста, попробуйте позже.',
      ...
      case 504:
        return {
          title: 'Таймаут шлюза',
          description: 'Сервер не получил своевременного ответа от вышестоящего сервиса. Попробуйте повторить запрос.',
  ...
  if (compact) {
    return (
      <div className={`flex items-center justify-between p-4 ${errorInfo.bgColor} border ${errorInfo.borderColor} rounded-lg`}>
...
export function ServerErrorInline({
  error,
  onRetry,
}: {
  error: any;

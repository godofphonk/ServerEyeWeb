      ...
      <div className='p-6'>
        <div className='flex items-start gap-4'>
          ...
          <div className='flex-1 space-y-3'>
            <div>
              <h3 className='text-lg font-semibold text-white mb-1'>{getErrorTitle()}</h3>

              <p className='text-gray-300 text-sm leading-relaxed'>{error.userMessage}</p>
            </div>
  ...
  return (
    <div className='flex items-center justify-between p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg'>
      <div className='flex items-center gap-3'>
        ...
        <div>
          <p className='text-sm text-white font-medium'>{error.userMessage}</p>
          {error.isTemporary && (
            <p className='text-xs text-gray-400 mt-1'>Временная проблема - попробуйте обновить</p>
          )}
        </div>
      </div>

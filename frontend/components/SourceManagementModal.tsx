...
export default function SourceManagementModal({
  ...
  const deleteIdentifiers = async (sourceType: string, identifiers: string[]) => {
    if (!confirm(`Are you sure you want to remove ${identifiers.length} identifier(s) from ${sourceType} source?`)) {
      return;
    }
    ...
    try {
      ...
      const response = await apiClient.deleteServerSourceIdentifiersByType(serverKey, sourceType, request);
      ...
      >
        ...
        >
          ...
          <div className="space-y-4">
            <h3 className="text-lg font-semibold text-white mb-4">Active Sources ({sources.length})</h3>
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        title="View details"
                      >
                        {showDetails[sourceType] ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                      </button>
                  ...
                  <AnimatePresence>
                    {showDetails[sourceType] && sourceDetails[sourceType] && (
                      ...
                      >
                        <h4 className="text-white font-semibold mb-3">Identifiers ({sourceDetails[sourceType].identifiers.length})</h4>
                          ...
                          {sourceDetails[sourceType].identifiers.map((identifier) => (
                            ...
                            >
                              <div className="flex-1">
                                <p className="text-white text-sm font-mono">{identifier.identifier}</p>
                                <p className="text-gray-400 text-xs">{identifier.identifier_type}</p>
                                {identifier.metadata && (
                                  <div className="mt-2">
                              ...
                              <button
                                onClick={() => deleteIdentifiers(sourceType, [identifier.identifier])}
                                disabled={isLoading}
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-gray-400 text-sm mt-1">
                  Removing sources will affect access to this server. Make sure you understand the implications before removing any source.
                </p>
              </div>
            </div>
          </div>
